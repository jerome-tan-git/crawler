...
public class lampym implements ISites {

	private final static Pattern SESSION = Pattern.compile(
			"http://www.lampym.com/course/[0-9]+",
	...
	public String[] getURLSeed() {
		...
		for (int i = 1; i <= pageLen; i++) {
			pages[i - 1] = "http://www.lampym.com/course/explore?page="+i;
		}
	...
	public String[] getFields(Document doc, Page page) {
		...
		//
		el = doc.select("div[class=teacher] div[class=col-md-10]");// teacher name
		if (el.size() > 0) {
			infoArray.add(el.get(0).text());
		} else {
...
//		} else
		...
		//
		el = doc.select("div[class=course-picture] img");// picture
		if (el.size() > 0) {
