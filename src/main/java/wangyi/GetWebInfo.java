...
//			}
		...
		for (int j = 0; j < threads; j++) {
			futures.add(pool.submit(new GetWebInfoThread(urlQueue, resultQueue, errUrlQueue)));
        }
		...
		for (String line:errUrlQueue) {
			logger.info("errUrl=" + line);
		}
...
class GetWebInfoThread implements Callable {
	...
	private ConcurrentLinkedQueue<String> errUrlQueue = null;
	public GetWebInfoThread(ConcurrentLinkedQueue<String> urlQueue, ConcurrentLinkedQueue<String> resultQueue, ConcurrentLinkedQueue<String> errUrlQueue) {
		this.urlQueue = urlQueue;
		this.resultQueue = resultQueue;
		this.errUrlQueue = errUrlQueue;
		getDriver();
	}
...
//			driver = new PhantomJSDriver(cap);
	...
	private boolean matchHtml(String url) {
		boolean flag = false;
		try {
			try {
				getDriver().get(url);
			...
			flag = true;
			System.out.println("process success. url=" + url);
		} catch (Exception e) {
			e.printStackTrace();
			errUrlQueue.add(url);
			System.out.println("matchHtml err, url=" + url);
			System.out.println("matchHtml err, msg=" + e.getMessage());
		}
		return flag;
	}
}
